...
void EBSL::predict(nb::ndarray<uint8_t, nb::numpy, nb::shape<-1>, nb::c_contig> out)
{
    ...
    {
        proba = run_once();
        // Rounding to 0 or 1 (not a general rounding)
        // Avoids excessive calls to round()
        results[current_iteration] = (proba >= 0.5);
    }
}
...
NB_MODULE(ebsl_cpp, m)
{
    ...
    nb::class_<Opinion>(m, "Opinion")
        .def(nb::init<>())
        .def(nb::init<float, float, float, float>(), "b"_a, "d"_a, "u"_a, "a"_a = 0)
        .def("__str__", &Opinion::to_string)
        .def("validate_opinions", &Opinion::validate_opinion)
        .def("set_parameters", &Opinion::set_parameters, "b"_a, "d"_a, "u"_a, "a"_a = 0)
